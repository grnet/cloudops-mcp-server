...
try:
    ...
    INIA_AVAILABLE = True
    logger.info(
        "inia module loaded successfully for email verification and password reset"
    )
except ImportError:
    INIA_AVAILABLE = False
    logger.warning(
        "inia module not available. Email verification and password reset tools will be disabled."
    )
...
def load_secrets_file() -> Dict[str, Any]:
    ...
    try:
        with open(secrets_path, "r", encoding="utf-8") as f:
            data = json.load(f)
            logger.info(
                f"Successfully loaded multi-institution AWS credentials from {secrets_path}"
            )
            return data
    ...
    try:
        ...
        for institution_name, credentials in institutions_credentials.items():
            if not credentials.get("aws_access_key_id") or not credentials.get(
                ...
                raise ValueError(
                    f"Missing required AWS credentials for institution '{institution_name}'"
                )

        logger.info(
            f"AWS credentials initialized successfully for {len(institutions_credentials)} institutions: {list(institutions_credentials.keys())}"
        )
        return True
...
def get_institution_credentials(institution_name: str) -> Optional[Dict[str, str]]:
    ...
    if institution_name not in institutions_credentials:
        logger.error(
            f"Institution '{institution_name}' not found. Available institutions: {list(institutions_credentials.keys())}"
        )
        return None
...
def get_aws_client(
    service: str, institution: str, region: str = "us-east-1"
) -> Optional[Any]:
    ...
    try:
        ...
        logger.info(
            f"Created AWS {service} client for institution '{institution}' in region {region}"
        )
        ...
        logger.error(
            f"Failed to create AWS client for {service}/{institution}/{region}: {e}"
        )
        return None
    ...
    if isinstance(data, dict):
        ...
        for key, value in data.items():
            if key.lower() in [
                ...
                sanitized[key] = [
                    sanitize_aws_response(item) if isinstance(item, dict) else item
                    for item in value
                ]
            else:
                sanitized[key] = value
...
def handle_aws_error(
    error: Exception, operation: str, institution: Optional[str] = None
) -> Dict[str, Any]:
    """Handle AWS API errors and return standardized error response."""
    error_response = {"success": False, "operation": operation}
    ...
    if isinstance(error, NoCredentialsError):
        ...
        error_response.update({"error": str(error), "error_type": "general_error"})
    ...
    try:
        ...
        def get_institution_resource(institution_id: str):
            ...
            try:
                    ...
                    try:
                        ...
                        return {
                            ...
                            "description": tags.get(
                                "Description", f"AWS Account {institution_id}"
                            ),
                            ...
                            "metadata": {
                                ...
                                "joined_timestamp": (
                                    account_info["JoinedTimestamp"].isoformat()
                                    if account_info.get("JoinedTimestamp")
                                    else None
                                ),
                                "tags": tags,
                ...
                raise ValueError(
                    f"Institution {institution_id} not found in any configured AWS organization"
                )

            except Exception as e:
                logger.error(
                    f"Error getting institution resource {institution_id}: {e}"
                )
                raise ValueError(
                    f"Institution {institution_id} not found or inaccessible"
                )
...
def get_institutions(
    ...
    try:
        ...
        logger.info(
            f"Getting institutions from AWS Organizations for '{institution}' - type: {institution_type}, details: {include_details}"
        )
        ...
        for account in accounts:
            ...
            if include_details:
                ...
                inst_data = {
                    ...
                    "type": account_type,
                    "description": tags.get(
                        "Description", f"AWS Account {account['Name']}"
                    ),
...
@mcp.tool()
def get_projects(
    institution: str, institution_id: str, include_aws_details: bool = False
) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        logger.info(
            f"Getting comprehensive projects for institution '{institution}' account: {institution_id}, AWS details: {include_aws_details}"
        )
        ...
        try:
            ...
            if not roots_response.get("Roots"):
                return {"success": False, "error": "No organization roots found"}
        ...
        for account in all_accounts:
            ...
            project_data = {
                ...
                "description": account_tags.get(
                    "Description", f"AWS Account: {account['Name']}"
                ),
                "budget": account_tags.get("Budget", "Not specified"),
                "services": (
                    account_tags.get("Services", "").split(",")
                    if account_tags.get("Services")
                    else []
                ),
                "ou_path": " > ".join(account_ou_path) if account_ou_path else "Root",
                "parent_id": account_parent,
                "account_type": account_tags.get(
                    "Type", account_tags.get("AccountType", "standard")
                ),
            ...
            if include_aws_details:
                project_data["aws_details"] = {
                    ...
                    "tags": account_tags,
                    "parent_type": (
                        "ROOT" if account_parent == root_id else "ORGANIZATIONAL_UNIT"
                    ),
        ...
        for ou in all_ous:
            ...
            try:
                ...
                ou_tags = {
                    tag["Key"]: tag["Value"] for tag in ou_tags_response.get("Tags", [])
                }
            except ClientError:
                ou_tags = {}
            ...
            try:
                accounts_in_ou_response = orgs_client.list_accounts_for_parent(
                    ParentId=ou_id
                )
            ...
            # Get child OUs count
            child_ous = [
                child_ou for child_ou in all_ous if child_ou.get("ParentId") == ou_id
            ]
            ...
            project_data = {
                ...
                "description": ou_tags.get(
                    "Description", f"Organizational Unit: {ou['Name']}"
                ),
                "budget": ou_tags.get("Budget", "Not specified"),
                "services": (
                    ou_tags.get("Services", "").split(",")
                    if ou_tags.get("Services")
                    else []
                ),
                ...
                "contains_target_account": institution_id
                in [acc["Id"] for acc in accounts_in_ou],
            }
            ...
            if include_aws_details:
                project_data["aws_details"] = {
                    ...
                    "tags": ou_tags,
                    "accounts_in_ou": [
                        {"id": acc["Id"], "name": acc["Name"]} for acc in accounts_in_ou
                    ],
                    "child_ous": [
        ...
        return {
            ...
            "data": {
                ...
                "summary": {
                    ...
                    "total_budget": (
                        total_budget if budget_specified_count > 0 else "Not calculated"
                    ),
                    "budget_specified_count": budget_specified_count,
                    "max_ou_level": (
                        max([ou.get("Level", 0) for ou in all_ous]) if all_ous else 0
                    ),
                },
    ...
    try:
        ...
        for permset_arn in permsets_response.get("PermissionSets", []):
            try:
                ...
                accounts_response = (
                    sso_admin_client.list_accounts_for_provisioned_permission_set(
                        InstanceArn=sso_arn, PermissionSetArn=permset_arn
                    )
                )
                ...
                for account_id in accounts_response.get("AccountIds", []):
                    try:
                        ...
                        for assignment in assignments_response.get(
                            "AccountAssignments", []
                        ):
                            ...
                            if principal_type == "GROUP":
                                assignments["Group"].setdefault(
                                    principal_id, []
                                ).append(account_id)
                            elif principal_type == "USER":
                                assignments["User"].setdefault(principal_id, []).append(
                                    account_id
                                )

                    except Exception as e:
                        logger.warning(
                            f"Could not get assignments for account {account_id}: {e}"
                        )
...
def _identify_group_owners(
    users: Dict[str, Any], groups: Dict[str, Any], assignments: Dict[str, Any]
) -> Dict[str, List[str]]:
...
def get_users(
    ...
    try:
        ...
        logger.info(
            f"Getting SSO users for institution '{institution}', role: {role_filter}, groups: {include_groups}, assignments: {include_assignments}"
        )
        ...
        assignments = (
            _fetch_sso_assignments(sso_admin_client, sso_arn)
        ...
        owners = (
            _identify_group_owners(users, groups, assignments) if include_groups else {}
        )
...
@mcp.tool()
def get_tags(
    institution: str, resource_arn: str, resource_type: Optional[str] = None
) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        logger.info(
            f"Getting tags for resource: {resource_arn} in institution '{institution}', type: {resource_type}"
        )

        # Parse ARN to determine service
        arn_parts = resource_arn.split(":")
        if len(arn_parts) < 6:
            return {"success": False, "error": "Invalid ARN format"}
        ...
        try:
            tagging_client = get_aws_client(
                "resourcegroupstaggingapi", institution, region
            )
            if tagging_client:
        ...
        if not tags:
            try:
                if service == "ec2":
                    ...
                    orgs_client = get_aws_client("organizations", institution)
                    if orgs_client:
                        response = orgs_client.list_tags_for_resource(
...
def _fetch_cost_explorer_data(
    ...
    try:
        ...
        if exclude_services:
            filters["Not"] = {
                "Dimensions": {"Key": "SERVICE", "Values": exclude_services}
            }
        ...
        if account_ids:
            ...
            if filters:
                filters = {"And": [filters, account_filter]}
            else:
...
def _analyze_project_budgets(
    costs: Dict[str, Any], budgets: Dict[str, float]
) -> Dict[str, Any]:
    ...
    for account_id, cost_data in costs.items():
        ...
        budget_status = (
            "overbudget" if actual_cost > budget and budget > 0 else "within_budget"
        )
    ...
    analysis["summary"]["budget_utilization"] = round(
        (
            (
                analysis["summary"]["total_cost"]
                / analysis["summary"]["total_budget"]
                * 100
            )
            if analysis["summary"]["total_budget"] > 0
            else 0.0
        ),
        2,
    )
    analysis["summary"]["remaining_budget"] = round(
        max(0, analysis["summary"]["total_budget"] - analysis["summary"]["total_cost"]),
        2,
    )
...
def _calculate_institution_costs(
    costs: Dict[str, Any], projects: List[Dict[str, Any]]
) -> Dict[str, Any]:
    ...
    return {
        ...
        "budget_utilization": round(
            (total_cost / total_budget * 100) if total_budget > 0 else 0.0, 2
        ),
        "top_services": [
            {"service": service, "cost": round(cost, 2)}
            for service, cost in top_services
...
def _identify_overbudget_projects(
...
def check_budget(
    ...
    try:
        ...
        logger.info(
            f"Checking budget for institution '{institution}', project: {project_id}, "
        ...
        try:
            ...
            return {"success": False, "error": f"Failed to fetch cost data: {str(e)}"}
        ...
        institution_totals = _calculate_institution_costs(
            processed_costs, cost_analysis["projects"]
        )
        ...
        if project_id and project_id in processed_costs:
            response_data["focused_project"] = {
                ...
                "project_name": project_metadata.get(project_id, {}).get(
                    "name", f"Account-{project_id}"
                ),
...
def _create_sso_client(institution: str) -> Optional[Any]:
    ...
    try:
        ...
        logger.error(
            f"Failed to create SSO client for institution '{institution}': {e}"
        )
        return None
...
def _get_identity_store_id(institution: str) -> Optional[str]:
    ...
    try:
        ...
        if not sso_admin_client:
            logger.error(
                f"Could not create SSO Admin client for institution '{institution}'"
            )
            return None
        ...
        logger.info(
            f"Found identity store ID '{identity_store_id}' for institution '{institution}'"
        )
        return identity_store_id

    except Exception as e:
        logger.error(
            f"Failed to get identity store ID for institution '{institution}': {e}"
        )
        return None


def _find_user_by_identifier(
    institution: str, user_identifier: str
) -> Optional[Dict[str, Any]]:
    """Find user ID by email/display name using AWS Identity Store directly."""
    try:
        logger.info(
            f"Searching for user '{user_identifier}' in institution '{institution}'"
        )
        ...
        if not identity_store_id:
            logger.error(
                f"Could not get identity store ID for institution '{institution}'"
            )
            return None

        # Get Identity Store client
        identitystore_client = get_aws_client(
            "identitystore", institution, "eu-central-1"
        )
        if not identitystore_client:
            logger.error(
                f"Could not create Identity Store client for institution '{institution}'"
            )
            return None
        ...
        try:
            ...
            for user in users:
                ...
                if (
                    user_identifier.lower() in user_email.lower()
                    or user_identifier.lower() in display_name.lower()
                    or user_identifier.lower() in username.lower()
                    or user_identifier == user_id
                ):
            ...
            logger.warning(
                f"User '{user_identifier}' not found in institution '{institution}'"
            )
            return None
        ...
        logger.error(
            f"Error searching for user '{user_identifier}' in institution '{institution}': {e}"
        )
        return None
...
@mcp.tool()
def verify_email(
    institution: str, user_identifier: str, user_id: Optional[str] = None
) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        logger.info(
            f"Starting email verification for user '{user_identifier}' in institution '{institution}'"
        )
...
def reset_password(
    ...
    try:
        ...
        logger.info(
            f"Starting password reset for user '{user_identifier}' in institution '{institution}' with mode '{mode}'"
        )
...
def initialize_server():
    ...
    try:
        logger.info(
            "Initializing Real AWS Organizations MCP Server with multi-institution support..."
        )
        ...
        available_institutions = get_available_institutions()
        logger.info(
            f"Server initialization completed successfully with {len(available_institutions)} institutions: {available_institutions}"
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--transport", choices=["http"], default="http", help="Transport type: http"
    )
    parser.add_argument(
    ...
    logger.info(
        f"Starting Real AWS Organizations MCP Server with multi-institution support in {args.transport.upper()} mode..."
    )
